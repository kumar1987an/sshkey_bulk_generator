...
if [ "$?" = 0 ]; then
        ...
        echo "'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''"
                if grep -w ""${Suser[x]}"@`uname -n`" id_rsa.pub; then
        echo "'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''"
                echo "KEY MATCHED WITH GIVEN PATTERN"
                if [[ $(cp -ip id_rsa.pub /tmp/"${Suser[x]}"_`uname -n`_id_rsa.pub) ]] > /dev/null 2>&1; then
        ...
        su - "${Suser[x]}" -c "echo |ssh-keygen -t rsa"
...
while [[ ".ssh/authorized_keys" == "${array[a]}" ]]; do
    ...
    if [[ "$?" = "0" ]]; then
        ...
        echo -e "ALREADY KEY(S) ARE THERE IN THE AUTH FILE, PLEASE CHECK THE SSH CONNECTION AND COME BACK\n"
                ...
                if [[ $(cat /tmp/"${Suser[x]}"_"${Sserver[x]}"_id_rsa.pub  >> authorized_keys) ]]; then printf "[CHECKOUT KEY ADDED AS BELOW]\n"; fi
...
while [[ ".ssh/authorized_keys2" == "${array[a]}" ]]; do
    ...
    if [[ "$?" = "0" ]]; then
        ...
        echo -e "ALREADY KEY(S) ARE THERE IN THE AUTH FILE, PLEASE CHECK THE SSH CONNECTION AND COME BACK\n"
                ...
                if [[ $(cat /tmp/"${Suser[x]}"_"${Sserver[x]}"_id_rsa.pub  >> authorized_keys2) ]]; then printf "[CHECKOUT KEY ADDED AS BELOW]\n"; fi
...
while [[ ".ssh/authorized_keys" != "${array[a]}" && ".ssh/authorized_keys2" != "${array[a]}" ]]; do
    ...
    if [[ $(ls -lrt $pattern) ]] > /dev/null 2>&1; then
         ...
         echo -e "ALREADY KEY(S) ARE THERE IN THE AUTH FILE, PLEASE CHECK THE SSH CONNECTION AND COME BACK\n"
                 ...
                 printf "File listing after backup of auth\n"
                 ls -lrt $pattern*
                 if [[ $(cat /tmp/"${Suser[x]}"_"${Sserver[x]}"_id_rsa.pub >> "$pattern") ]]; then printf "[CHECKOUT KEY ADDED AS BELOW]\n"; fi
                 tail -1 "$pattern";
                 ls -lrt "$pattern"
         fi
    else
...
Suser[x]=$1
Sserver[x]=$2
rm -f /tmp/"${Suser[x]}"_"${Sserver[x]}"_id_rsa.pub
while [[ "$?" = 0  ]]; do
...
scp "${Sserver[x]}":/tmp/"${Suser[x]}"_"${Sserver[x]}"_id_rsa.pub /tmp
if [[ "$?" = 0 ]]; then
echo -e "FETCHED FILE from source server to jump server\n"
scp /tmp/"${Suser[x]}"_"${Sserver[x]}"_id_rsa.pub "${Tserver[x]}":/tmp/
